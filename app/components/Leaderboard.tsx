    ...
    const sorted = [...leaderboardData].sort((a, b) => {
      ...
      switch (sortColumn) {
        ...
        case "o2GoalsPct":
          ...
          break;
        case "avgOdds":
          aValue = parseFloat(a.avgOdds);
          bValue = parseFloat(b.avgOdds);
          break;
        case "avgWinningOdds":
          aValue = parseFloat(a.avgWinningOdds);
          bValue = parseFloat(b.avgWinningOdds);
          break;
        default:
          return 0;
      }
          ...
          <table className="w-full min-w-[800px]">
            <thead>
              <tr className="bg-gradient-to-r from-slate-700 to-slate-600 border-b border-slate-600">
                <th className="sticky left-0 z-20 px-1 md:px-2 py-2 md:py-4 text-left text-xs md:text-sm font-semibold text-slate-200 bg-slate-700">
                  Rank
                </th>
                <th
                ...
                >
                  <div className="flex items-center justify-center gap-1">
                    Avg Odds <SortIcon column="avgOdds" />
                  </div>
                </th>
                <th
                  onClick={() => handleSort("avgWinningOdds")}
                  className="px-2 md:px-6 py-2 md:py-4 text-center text-xs md:text-sm font-semibold text-slate-200 cursor-pointer hover:bg-slate-600 transition-colors select-none"
                >
                  <div className="flex items-center justify-center gap-1">
                    Avg Winning Odds <SortIcon column="avgWinningOdds" />
                  </div>
                </th>
                <th
            ...
            <tbody>
              {getSortedData().map((u, index) => (
                <tr
                  key={u.user}
                  className={`border-b border-slate-700 transition-colors ${
                    index % 2 === 0 ? "bg-slate-800" : "bg-slate-750"
                  } hover:bg-slate-700 ${
                    u.user === selectedPlayer
                ...
                >
                  ...
                  <td className="sticky left-[32px] md:left-[48px] z-10 px-2 md:px-6 py-2 md:py-4 text-xs md:text-sm font-medium text-white bg-slate-800">
                    {u.user}
                  </td>
                  <td className="px-2 md:px-6 py-2 md:py-4 text-xs md:text-sm text-center font-bold">
                    <span
                      className={`${getWinRateColor(u.winPct)} text-white px-3 py-1 rounded-full text-xs`}
                    >
                      {u.winPct}%
                    </span>
                  </td>
                  ...
                  <td className="px-2 md:px-6 py-2 md:py-4 text-xs md:text-sm text-center text-green-400 font-semibold">
                    {u.wins}
                  </td>
                  <td className="px-2 md:px-6 py-2 md:py-4 text-xs md:text-sm text-center text-red-400 font-semibold">
                    {u.losses}
                  </td>
                  <td className="px-2 md:px-6 py-2 md:py-4 text-xs md:text-sm text-center">
                  ...
                  <td className="px-2 md:px-6 py-2 md:py-4 text-xs md:text-sm text-center">
                    <span
                      className={`${u.currentStreak > 0 ? "bg-green-600" : "bg-red-600"} text-white px-2 md:px-3 py-1 rounded-full text-xs font-bold`}
                    >
                      {(u.currentStreak > 0 ? "🔥" : "💀") +
                        Math.abs(u.currentStreak)}
                    </span>
                  </td>
                  <td className="px-2 md:px-6 py-2 md:py-4 text-xs md:text-sm text-center">
