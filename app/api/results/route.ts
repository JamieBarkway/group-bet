...
function extractScores(result: any): { home: number | null; away: number | null; eventId?: string } {
	// Try common fields
	const fields = [
	...
	const eventId = result?.eventId || result?.id || result?.event_id || undefined;
	return { home, away, eventId };
}

function decideWin(type: string, home: number | null, away: number | null): "W" | "L" | null {
...
function recalcEmojis(results: Array<{ outcome: "W" | "L" | "P"; emoji: string | null }>, specialEmojis?: Record<number, string>) {
...
export async function POST() {
	try {
		...
		const pending: Array<{ userIndex: number; resultIndex: number; type: string; eventId: string; start: Date }>= [];
		for (let ui = 0; ui < users.length; ui++) {
			const u = users[ui];
			for (let ri = 0; ri < u.results.length; ri++) {
				const r = u.results[ri];
				if (r.outcome === "P" && r.prediction?.match?.eventId && r.prediction?.match?.startDateTimeUtc) {
					pending.push({
						userIndex: ui,
						resultIndex: ri,
		...
		if (pending.length === 0) {
			return NextResponse.json({ settled: 0, message: "No pending predictions" });
		}

		const latestKickoff = new Date(Math.max(...pending.map((p) => p.start.getTime())));
		const now = new Date();
		const twoHoursMs = 2 * 60 * 60 * 1000;
		if (now.getTime() - latestKickoff.getTime() < twoHoursMs) {
			return NextResponse.json({ settled: 0, message: "Too early to settle; latest game not 2h old" });
		}

		// Fetch all leagues results
		const leaguePromises = LEAGUES.map((l) => fetchLeagueResults(l.name, l.endpoint).catch(() => []));
		const sets: any[][] = await Promise.all(leaguePromises);
		const allResults: any[] = sets.flat();

		// Build result map by eventId
		const byId = new Map<string, { home: number | null; away: number | null }>();
		...
		for (const p of pending) {
			...
			if (target.prediction) {
				target.prediction.finalScore = { home: scores.home, away: scores.away } as any;
			}
			settled += 1;
		}
		...
		for (const p of pending) {
			...
			if ((p.type === "BTTS" || p.type === "O2.5") && scores.home === 0 && scores.away === 0) {
		...
		// Check for 🤢: only loser in the round (find the round index)
		if (pending.length > 0) {
			const roundIndex = pending[0].resultIndex; // All pending should be same round
			const losersInRound = pending.filter(
				(p) => users[p.userIndex].results[p.resultIndex].outcome === "L" && p.resultIndex === roundIndex
			);
			if (losersInRound.length === 1) {
				const loser = losersInRound[0];
				// Only add 🤢 if they don't already have a special emoji
				if (!specialEmojis[loser.userIndex]?.[loser.resultIndex]) {
					if (!specialEmojis[loser.userIndex]) specialEmojis[loser.userIndex] = {};
					specialEmojis[loser.userIndex][loser.resultIndex] = "🤢";
				}
			}
		}

		// Recalculate emojis per user, passing special emojis
		users.forEach((u, userIndex) => recalcEmojis(u.results, specialEmojis[userIndex]));

		// Persist results
		fs.writeFileSync(DATA_PATH, JSON.stringify(users, null, 2));

		return NextResponse.json({ settled, message: `Settled ${settled} predictions` });
	} catch (error) {
		return NextResponse.json(
